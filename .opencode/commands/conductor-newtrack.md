...
2.  **Get Track Description & Enter Plan Mode:**
    *   **If `{{args}}` is empty:**
        1. Call the `transition into planning mode by updating plan.md` tool with the reason: "Defining new track".
        2. Ask the user using the `question` tool (do not repeat the question in the chat):
            - **questions:**
                - **header:** "Description"
                - **type:** "text"
    ...
    *   **If `{{args}}` contains a description:**
        ...
        2. Call the `transition into planning mode by updating plan.md` tool with the reason: "Defining new track".
...
2.  **Questioning Phase:** Ask a series of questions to gather details for the `spec.md` using the `question` tool. You must batch up to 4 related questions in a single tool call to streamline the process. Tailor questions based on the track type (Feature or Other).
    *   **CRITICAL:** Wait for the user's response after each `question` tool call.
    *   **General Guidelines:**
        ...
        *   **2. Formulate the Question:** Use the `question` tool: Adhere to the following for each question in the `questions` array:
        ...
        *   **3. Interaction Flow:**
            *   Wait for the user's response after each `question` tool call.
            *   If the user selects "Other", use a subsequent `question` tool call with `type: "text"` to get their input if necessary.
            *   Confirm your understanding by summarizing before moving on to drafting.

    *   **If FEATURE:**
        *   **Ask 3-4 relevant questions** to clarify the feature request using the `question` tool.
    ...
    *   **If SOMETHING ELSE (Bug, Chore, etc.):**
        *   **Ask 2-3 relevant questions** to obtain necessary details using the `question` tool.
