...
1.  **Enter Plan Mode:** Call the `transition into planning mode by updating plan.md` tool with the reason: "Setting up Conductor project".
...
-   **If Brownfield:**
        ...
        -   **Begin Brownfield Project Initialization Protocol:**
            -   **1.0 Pre-analysis Confirmation:**
                ...
                2.  **Ask for Permission:** Request permission for a read-only scan to analyze the project using the `question` tool:
                    - **header:** "Permission"
...
5.  **Inquire about Project Goal (for Greenfield):**
    -   **Ask the user the following question using the `question` tool and wait for their response before proceeding to the next step:**
        - **header:** "Project Goal"
        - **type:** "text"
...
2.  **Determine Mode:** Use the `question` tool to let the user choose their preferred workflow.
    - **questions:**
        - **header:** "Product"
...
4.  **Gather Information (Conditional):**
    ...
    -   **If user chose "Interactive":** Use a single `question` tool call to gather detailed requirements (e.g., target users, goals, features).
...
5.  **User Confirmation Loop:**
    -   **Ask for Approval:** Use the `question` tool to request confirmation. You MUST embed the drafted content directly into the `question` field so the user can review it in context.
        - **questions:**
            - **header:** "Review Draft"
            - **question:**
...
3.  **Gather Information (Conditional):**
    ...
    -   **If user chose "Interactive":** Use a single `question` tool call to gather detailed preferences.
        -   **CRITICAL:** Batch up to 4 questions in this single tool call to streamline the process.
...
2.  **Determine Mode:**
    -   **FOR GREENFIELD PROJECTS:** Use the `question` tool to choose the workflow.
        - **questions:**
            - **header:** "Tech Stack"
    ...
    -   **FOR BROWNFIELD PROJECTS:**
        ...
        -   **Request Confirmation:** After stating the detected stack, you MUST ask the user for confirmation using the `question` tool:
            - **questions:**
                - **header:** "Tech Stack"
                - **question:** "Is the inferred tech stack (listed above) correct?"
                - **type:** "yesno"
        -   **Handle Disagreement:** If the user answers 'no' (disputes the suggestion), you MUST immediately call the `question` tool with `type: "text"` to allow the user to provide the correct technology stack manually. Once provided, proceed to draft the document using the user's input.
...
2.  **Select Code Style Guides:**
    ...
    -   **FOR GREENFIELD PROJECTS:**
        ...
        -   **Determine Mode:** Use the `question` tool:
            - **questions:**
        ...
        -   **If user chose "Select from Library":**
            ...
            -   **Action:** Announce "I'll present the available guides in groups. Please select all that apply." Then, immediately call the `question` tool with the batched questions (do not list the questions in the chat).
            -   **Single Tool Call:** Create one `question` call containing a `questions` array with one question per group.
    ...
    -   **FOR BROWNFIELD PROJECTS:**
        ...
        -   **If user chose "Add More":**
            -   **Action:** Announce "I'll present the additional guides. Please select all that apply." Then, immediately call the `question` tool (do not list the questions in the chat).
            -   **Method:** Use a single `question` tool call. Dynamically split the available guides into batches of 4 options max. Create one `multiSelect: true` question for each batch.
...
3.  **Gather Information (Conditional):**
    ...
    -   **If user chose "Customize":**
        a. **Initial Batch:** Use a single `question` tool call to gather primary customizations:
            - **questions:**
                - **header:** "Coverage"
        ...
        b. **Final Tweak (Second Batch):** Once the first batch is answered, immediately use a second `question` tool call to show the result and allow for any additional tweaks:
            - **questions:**
                - **header:** "Workflow"
                - **type:** "text"
                - **question:**
...
3.  **Determine Mode:** Use the `question` tool to let the user choose their preferred workflow.
    - **questions:**
        - **header:** "Product Reqs"
...
5.  **Gather Information (Conditional):**
    ...
    -   **If user chose "Interactive":** Use a single `question` tool call to gather detailed requirements.
...
7.  **User Confirmation Loop:**
    ...
    -   **Ask for Approval:** Use the `question` tool to request confirmation. You MUST embed the drafted requirements directly into the `question` field so the user can review them.
        - **questions:**
            - **header:** "Review"
            - **question:**
...
3.  **Confirm Proposal:** Use the `question` tool to validate the proposal:
    - **questions:**
...
4.  **Action:**
    ...
    -   **If user chose "Suggest changes":**
        -   Immediately call the `question` tool again:
            - **header:** "New Track"
            - **type:** "text"
