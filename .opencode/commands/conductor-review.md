...
2.  **Auto-Detect Scope:**
    ...
    -   If one exists, immediately call the `question` tool to confirm (do not repeat the question in the chat):
        - **questions:**
            - **header:** "Review Track"
            - **question:** "Do you want to review the in-progress track '<track_name>'?"
            - **type:** "yesno"
    -   If no track is in progress, or user says "no", immediately call the `question` tool to ask for the scope (do not repeat the question in the chat):
        - **questions:**
            - **header:** "Select Scope"
            - **question:** "What would you like to review?"
            - **type:** "text"
            - **placeholder:** "Enter track name, or 'current' for uncommitted changes"
3.  **Confirm Scope:** Ensure you and the user agree on what is being reviewed by immediately calling the `question` tool (do not repeat the question in the chat):
    - **questions:**
...
3.  **Load and Analyze Changes (Smart Chunking):**
    ...
    -   **Strategy Selection:**
        ...
        -   **Large Changes (> 300 lines):**
            -   **Confirm:** Immediately call the `question` tool to confirm before proceeding with a large review (do not repeat the question in the chat):
                - **questions:**
                    - **header:** "Large Review"
...
2.  **Action:**
    -   **If issues found:** Immediately call the `question` tool (do not repeat the question in the chat):
        - **questions:**
            - **header:** "Decision"
...
2.  **Condition for Action:**
    ...
    -   If changes are detected:
        a. **Check for Track Context:**
            - If you are NOT reviewing a specific track (i.e., you don't have a `plan.md` in context), immediately call the `question` tool (do not repeat the question in the chat):
                - **questions:**
        ...
        b. **Handle Track-Specific Changes:**
            i.   **Confirm with User:** Immediately call the `question` tool (do not repeat the question in the chat):
                - **questions:**
                    - **header:** "Commit & Track"
...
3.  **Handle User Response:**
    ...
    *   **If "Delete":**
        i.   **Confirm:** Immediately call the `question` tool to ask for final confirmation (do not repeat the warning in the chat):
            - **questions:**
                - **header:** "Confirm"
