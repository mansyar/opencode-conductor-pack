...
3.  **Interaction Paths:**
    ...
    *   **PATH A: Direct Confirmation**
        ...
        2.  Immediately call the `question` tool to confirm the selection (do not repeat the question in the chat):
            - **questions:**
                - **header:** "Confirm"
                - **question:** "You asked to revert the [Track/Phase/Task]: '[Description]'. Is this correct?"
                - **type:** "yesno"
        3.  If "yes", establish this as the `target_intent` and proceed to Phase 2. If "no", immediately call the `question` tool to ask clarifying questions (do not repeat the question in the chat):
            - **questions:**
                - **header:** "Clarify"
    ...
    *   **PATH B: Guided Selection Menu**
        ...
        2.  **Present a Unified Hierarchical Menu:** Immediately call the `question` tool to present the results (do not list them in the chat first):
            - **questions:**
                - **header:** "Select Item"
        ...
        3.  **Process User's Choice:**
            ...
            *   If the user selects "Other" (automatically added for "choice") or the explicit "Other" option provided, you must engage in a dialogue to find the correct target using `question` tool with a single question of `type: "text"` in the `questions` array.
...
2.  **Final Go/No-Go:** Immediately call the `question` tool to ask for final confirmation (do not repeat the question in the chat):
    - **questions:**
        - **header:** "Confirm Plan"
...
3.  **Process User Choice:**
    ...
    - If "Revise", immediately call the `question` tool to get the correct plan (do not repeat the question in the chat):
        - **questions:**
            - **header:** "Revise"
